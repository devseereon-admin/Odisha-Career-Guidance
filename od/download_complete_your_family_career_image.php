...
$font_size = 19;
$line_height = 23; // Adjust as needed to control spacing between lines
...
function addTextBlock($image, $text, $x, $y, $fontPath, $fontSize, $lineHeight, $textColor, $padding = 0) {
    ...
    foreach ($lines as $line) {
        ...
        $textY = $y + $padding + $lineHeight; // Y-coordinate for text (top padding)
...
$prefix = [
    "Me", "Mother's Career", "Father's Career", "Brother/Cousin's Career", 
    "Sister/Cousin's Career", "P. Uncle's Career", "P. Aunt's Career", 
    "M. Uncle's Career", "M. Aunt's Career", "P. Grandfather's Career", 
    "P. Grandmother's Career", "M. Grandfather's Career", "M. Grandmother's Career"
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($positions as $key => $position) {
        $postKey = 'designation' . ($key + 1);
        $relationer_name = 'name' . ($key + 1);
        if (isset($_POST[$postKey]) && !empty($_POST[$postKey])) {
            // Combine prefix and posted data with a newline
            $text = $prefix[$key] . ": ".$_POST[$relationer_name]." \n" . $_POST[$postKey];
            ...
            addTextBlock($base_image, $text, $x, $y, $font_path, $font_size, $line_height, $text_color, 20);
        }
    }
