...
include "dbconn.php";
$eid = $_GET['id'];
$exam_det = mysqli_fetch_assoc(mysqli_query($conn , "select * from `entnace_exam` where id='$eid' and status='1' "));
$type = $exam_det['type'];
$loc = $exam_det['location'];
$qual = $exam_det['qualification'];
$qual_exe = mysqli_query($conn ,"select * from qualification where id='$qual' and status='1'");
...
if (!empty($_POST['action']) && $_POST['action'] == "update") {
	...
	//var_dump($_POST);
		...
		//echo "UPDATE  `catagory` SET `name` ='$name' , `icon`='$strm_img' , `priority` ='$priority' WHERE `id`='$id' " ; 
		//exit;
$insert = mysqli_query($conn , "UPDATE  `entnace_exam` SET `name` ='$name' , `qualification`='$qualification' , `type` ='$institute_typ' , `location`='$location' , `description`='$about',`link`='$link'  WHERE `id`='$id' ");
...
if($insert){
        ...
        $msg = "Unable to update exam";
        $alert = "danger";
    }

	
	
	header("Location:entrance_exm.php");
}
?>
<!DOCTYPE html>
<html class=" ">
...
<div class="col-xl-12 col-lg-12 col-12 col-md-12">
                        <section class="box ">
                            <header class="panel_header">
                                <h2 class="title float-left">Edit Exam</h2>
                            ...
                            <div class="content-body">
                                <div class="row">
									...
									<form name="strmfrm" method="post" action="edit_exam.php" enctype="multipart/form-data">
									...
									<input type="hidden" name="id" value="<?=$eid;?>">
                                       <div class="form-group">
                                            ...
                                            <div class="controls">
												...
												<option value="3">Competitive Exams for job</option>
												<?php
												}elseif($type==2){
												?>
												<option value="1">Under Graduate</option>
												<option value="2" selected>Post Graduate</option>
												<option value="3">Competitive Exams for job</option>
												<?php
												}elseif($type==3){
												?>
												<option value="1">Under Graduate</option>
												<option value="2">Post Graduate</option>
												<option value="3" selected>Competitive Exams for job</option>
												<?php
												}
												?>
                                       ...
                                       <div class="form-group">
                                            <label class="form-label" for="field-2">Qualification</label>
                                           
                                             <select class="custom-select" id="inputGroupSelect01" name="qualification">
                                                <option selected>Choose...</option>
												<?php
												$strm_sql = mysqli_query($conn , "select * from qualification where status='1'");
												...
												?>
                                                <option value="<?=$resstrm['id'];?>" <?=$select;?>><?=$resstrm['name'];?></option>
												<?php
												}
												?>
									...
									if($loc == 1){
									?>
										<input type="radio" name="location" id="National" onclick="show1()" value="1" checked> 
										Odisha
										<input type="radio"  name="location" id="Internatinal" onclick="show2()" value="2">
										All India
										<?php
									}elseif($loc==2){
										?>
										<input type="radio" name="location" id="National" onclick="show1()" value="1" > 
										Odisha
										<input type="radio"  name="location" id="Internatinal" onclick="show2()" value="2" checked>
										All India
<?php
									}
?>										
