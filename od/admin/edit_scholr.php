...
$det_schlr = mysqli_query($conn ,"SELECT * FROM scholarship WHERE id='$id' AND status='1'");
$res_det = mysqli_fetch_assoc($det_schlr);

$type  = $res_det['type'];
$class = $res_det['class'];
$name  = $res_det['name'];
$link  = $res_det['link'];
$description = $res_det['description'];
$stipend = $res_det['stipend'];
$eligibility_criteria = $res_det['eligibility_criteria'];
...
if (!empty($_POST['action']) && $_POST['action'] == "update") {
    ...
    // If type is not State then class = 0
    if($type != 2){
        $class = 0;
    }
    ...
    $eligibility_criteria = mysqli_real_escape_string($conn, $eligibility_criteria);

    $update = mysqli_query($conn, "UPDATE scholarship SET
        name='$name',
        class='$class',
        type='$type',
        link='$link',
    ...
    if($update){
        ...
        $msg = "Unable to update scholarship";
        $alert = "danger";
    }
                ...
                <section class="wrapper main-wrapper" style=''>
                    ...
                    <div class='col-xl-12 col-lg-12 col-md-12 col-12'>
                        <div class="page-title">
                            ...
                            <div class="float-left">
                                <h1 class="title">Scholarships</h1>                            </div>
...
<div class="col-xl-12 col-lg-12 col-12 col-md-12">
                        <section class="box ">
                            <header class="panel_header">
                                <h2 class="title float-left">Add Scholarship</h2>
                            ...
                            <div class="content-body">
                                <div class="row">
									...
									<form name="strmfrm" method="post" action="edit_scholr.php" enctype="multipart/form-data">
									...
									<input type= "hidden" name="sid" value="<?=$id;?>">
										...
										<div class="form-group">
                                            ...
                                            <div class="controls">
<select class="custom-select" name="type" onchange="getclass(this.value)">

<option value="">Select Type</option>

<option value="1" <?=($type==1)?'selected':''?>>Central</option>
<option value="2" <?=($type==2)?'selected':''?>>State</option>
<option value="3" <?=($type==3)?'selected':''?>>Private</option>
<option value="4" <?=($type==4)?'selected':''?>>PSU</option>

</select>
                                            </div>
										</div>
										
										<div id="cls" style="<?=($type==2)?'display:block':'display:none';?>">

<select class="custom-select" name="class">

<option value="">Select Class</option>

<option value="1" <?=($class==1)?'selected':''?>>1st - 5th</option>
<option value="2" <?=($class==2)?'selected':''?>>6th - 8th</option>
<option value="3" <?=($class==3)?'selected':''?>>9th - 10th</option>
<option value="4" <?=($class==4)?'selected':''?>>11th - 12th</option>
<option value="5" <?=($class==5)?'selected':''?>>Under Graduate</option>
<option value="6" <?=($class==6)?'selected':''?>>Post Graduate</option>
										...
										<div class="form-group">
                                             <label class="form-label" for="field-2">Eligibility Criteria</label>
                                         
                                            <div class="controls">
											
                                                <textarea class="ckeditor" cols="80" id="editor2" name="eligibility_criteria" rows="10">
										...
										<div class="form-group">
                                             <label class="form-label" for="field-2">Stipend</label>
                                         
                                            <div class="controls">
											
                                                <textarea class="ckeditor" cols="80" id="editor3" name="stipend" rows="10">
												<?=$stipend;?>
												</textarea>
										...
										<button type="submit" value="submit" class="btn btn-success btn-corner">update</button>
									</form>
                                    </div>
                                </div>
...
{
    var classBox = document.getElementById("cls");
    ...
    if(val == 2){
        ...
        classSelect.value = "0"; // reset class
    }
}
</script>
    </body>
