        ...
        <div class="page-container row-fluid">
            ...
            <section id="main-content">
                <section class="wrapper main-wrapper">
                        ...
                        <div class="page-title">
                            ...
                            <div class="float-right d-none">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="active">
                                        <strong>View all carrer search </strong>
                                    </li>
                                </ol>
                            </div>
                    ...
                    <div class="clearfix">User Career Count List</div>
                    <div class="col-xl-12">
                        <section class="box">
                            <header class="panel_header">
                                <h2 class="title float-left">List Of Career</h2>
                                <div class="actions panel_actions float-right">
                                   <a href="dashboard.php" style="background-color:#0f3970;color:#fff;padding: 10px;border-radius: 10px;">Dashboard</a>
                                </div>
                            </header>
                                ...
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-12 padding-0">
                                        <div class="table-container">
                                            <table id="example-11" class="table table-striped dt-responsive display" width="100%">
                                            <thead id="table-head"></thead>
                                            <tbody id="result-append"></tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
...
function createPageFilter()
{
    ...
    $("#page-filter-box").remove();

    let pageFilter = `
    <span id="page-filter-box" style="margin-right:15px;">
        Page :
        <select id="page-from" style="padding:4px;">
            <option value="">All Pages</option>
            <?php
            $stm_sql = mysqli_query($conn,"SELECT DISTINCT page_from FROM career_save_details");
            while($res_stm = mysqli_fetch_array($stm_sql))
            {
                echo "<option value='".$res_stm['page_from']."'>".$res_stm['page_from']."</option>";
            }
            ?>
        </select>
    </span>
    ...
    $("#example-11_filter").prepend(pageFilter);

    // keep selected value
    $("#page-from").val(currentPage);

    $("#page-from").on("change", function(){
        currentPage = $(this).val();
        loadTable(currentPage);
    });
}

function loadTable(page_from = '')
{
    if ($.fn.DataTable.isDataTable('#example-11')) {
        $('#example-11').DataTable().destroy();
    }
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            $("#table-head").html(res.header);
            $("#result-append").html(res.body);

            table = $('#example-11').DataTable({
                pageLength:10,
