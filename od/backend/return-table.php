    ...
    if($option == 100)
    {
        ...
        $html = "
            <div class='form-group'>
                <label class='control-label'>ଆପଣଙ୍କ ସ୍ଥାନ ଚୟନ କରନ୍ତୁ</label><br>
                <div class='custom-control custom-radio'>
                    <input type='radio' name='location' id='location' value='0' checked> ଜାତୀୟ
                </div>
            </div>";
            
            $html .= "
            <select class='form-select' name='State' id='State' required onchange='showhidestate(this.value);'>
                <option value>ରାଜ୍ୟ ଚୟନ କରନ୍ତୁ</option>";
    ...
    else if($option == 101)
    {
        $html = "
            <select class='form-select' name='dist' id='dist' required >
                <option value>ଜିଲ୍ଲା ଚୟନ କରନ୍ତୁ</option>";
    ...
    else if($option ==102)
    {
        ...
        if($state_sql->num_rows <1)
        {
            $html .= "<div class='content' style='padding-bottom:7px;'>";
                $html .= "<h4 style='cursor:pointer;' '>ଆମେ ଖୁବ୍ ଶୀଘ୍ର ତଥ୍ୟ ପ୍ରଦାନ କରିବୁ</h4>";
        ...
        else
        {
            while ($row = mysqli_fetch_array($state_sql)) {
                ...
                $html .= "<a href='javascript:void(0)' onclick='viewInstitutionDetails($id)'  style='cursor:pointer;'>ଅଧିକ ଜାଣନ୍ତୁ</a>&nbsp;&nbsp;<a href='$link' target='_blank'>ପରିଦର୍ଶନ କରନ୍ତୁ</a>";
                $html .= "</div>";
            }
        }
    ...
    else if($option == 105)
    {
        ...
        $html = "
            <select class='form-select' name='subcat' id='subcat' required >
                <option value>ବିକଳ୍ପ ଚୟନ କରନ୍ତୁ</option>";
            
            $state_sql = mysqli_query($conn, "select * from `subcatagory` where `cat_id`='6' and status='1'");
            while ($res_stat = mysqli_fetch_array($state_sql)) {
                $html .= "<option value='" . $res_stat['id'] . "'>" . $res_stat['name'] . "</option>";
            }
