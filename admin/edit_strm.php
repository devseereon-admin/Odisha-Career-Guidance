...
include "dbconn.php";
$cid = $_GET['id'];
$carr_det = mysqli_fetch_assoc(mysqli_query($conn , "select * from `catagory` where id='$cid' and status='1' "));

if (!empty($_POST['action']) && $_POST['action'] == "update") {
	extract($_POST);
	$carr_dett = mysqli_fetch_assoc(mysqli_query($conn , "select * from `catagory` where id='$id' and status='1' "));
	
	
	if($_FILES["strm_img"]["name"] !=''){
		$target_dir = "../stream-img/";
	$target_file = $target_dir . basename($_FILES["strm_img"]["name"]);
	$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
	$strm_img=rand(1,999999)."".time().".".$imageFileType;
	$chk=move_uploaded_file($_FILES["strm_img"]["tmp_name"], $target_dir.$strm_img);
	}else{
		$strm_img = $carr_dett['icon'];
	}
	
	if($chk){
$insert = mysqli_query($conn , "UPDATE  `catagory` SET `name` ='$name' , `icon`='$strm_img' , `priority` ='$priority' WHERE `id`='$id' ");
...
<div class="col-xl-12 col-lg-12 col-12 col-md-12">
                        <section class="box ">
                            <header class="panel_header">
                                <h2 class="title float-left">Add Streams</h2>
                            ...
                            <div class="content-body">
                                <div class="row">
									...
									<form name="strmfrm" method="post" action="edit_strm.php" enctype="multipart/form-data">
									...
									<input type="hidden" name="id" value="<?=$cid;?>">
                                        <div class="form-group">
                                            ...
                                            <div class="controls">
                                                <input type="text" class="form-control" id="field-1" name="name" value="<?=$carr_det['name'];?>">
                                            </div>
                                        </div>
										...
										if($carr_det['icon']==''){
										?>
                                                 <input type="file" value="" class="form-control" id="field-2" name="strm_img" >
												<?php
										}else{
												?>
										<input type="file" class="form-control" id="field-2" name="strm_img" ><br/><br/>
										<img src="../stream-img/<?=$carr_det['icon']?>" name="prev_strm_img" id="prev_strm_img"/ style="width:100px;height:100px;">
										 ...
										 <div class="form-group">
                                            <label class="form-label" for="field-2">priority</label>
                                           
                                            <input type="text" class="form-control" id="field-1" name="priority" value="<?=$carr_det['priority'];?>">
                                        </div>
