...
if (!empty($_POST['action']) && $_POST['action'] == "add") {
	...
	$sub_subcat_det = mysqli_fetch_assoc(mysqli_query($conn ,"select * from `sub_subcategory` where id='$sid' and status='1' "));
	$sub_subcatname = $sub_subcat_det['name'];
	

	
$insert = mysqli_query($conn , "INSERT INTO `detail` (`sid`, `name`, `about`, `road_map`, `clg_in`, `clg_od`, `clg_ab`, `enxm_in`, `enxm_od`, `enxm_ab`, `schlr_od`, `schlr_national`, `enterprener`, `differenty_abled`,`aboiut_img`, `skill_required`, `success_strimg`, `sucees_stitle`, `success_slink`, `success_cont`,`enterpre_succ_titl`, `enterpre_succ_img`, `enterpre_succ_con`, `enterpre_succ_source`,`banner`) 
 VALUES ('$sid' ,'$sub_subcatname' ,'$about','$road_map','$clg_in','$clg_od','$clg_ab','$enxm_in','$enxm_od','$enxm_ab','$schlr_od','$schlr_national','$enterprener','$differenty_abled','$abt_img','$skill_required' ,'$success_strimg','$sucees_stitle','$success_slink','$success_cont','$enterpre_succ_titl','$enterpre_succ_img','$enterpre_succ_con','$enterpre_succ_source','$banner')");
//exit;
if($insert){
...
if (!empty($_POST['action']) && $_POST['action'] == "update") {
	...
	$sub_sub_cat_det = mysqli_fetch_assoc(mysqli_query($conn , "select * from detail where `sid`='$sid' and status='1'"));
...
$update = mysqli_query($conn ,"UPDATE `detail` SET `about`='$about',`road_map`='$road_map',`clg_in`='$clg_in',
...
`enterpre_succ_con`='$enterpre_succ_con',`enterpre_succ_source`='$enterpre_succ_source' ,`banner`='$banner' WHERE sid='$sid' ");
