...
function getInstitute(page)
{
  ...
  function getDistrict(ditrid){
	...
	function getDistrict_with_hidden(ditrid){
		$.ajax({
			type:'POST',
			url:"get_District.php",
			data:{ditrid:ditrid},
...
// }
  ...
  function getblock(blockid){
    ...
    function showAllOptions(selectId) {
        getDistrict_with_hidden(1);
        // document.getElementById('distdiv').innerHTML =`<select onclick='getDistrict(1)' class="form-select" name="District"><option>Select District</option></select>`;
      const select = document.getElementById(selectId);
      for (let option of select.options) {
          if(option.value == 0)
          {
